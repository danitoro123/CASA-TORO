...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (u) {
        setUser(u)
        setRol(u.user_metadata?.rol ?? 'lectora')
  ...
  if (checking) return (
    <html lang="es">
      <head><title>Casa Toro</title></head>
      <body style={{ background: '#f8f7f5', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <div style={{ width: 32, height: 32, border: '2px solid #d49a3a', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      </body>
    </html>
  )
      ...
      <body className="flex min-h-screen bg-stone-50">
        <aside className="w-56 shrink-0 bg-[#2e1e0b] text-stone-200 flex flex-col py-8 px-5 min-h-screen">
          <div className="mb-10">
            ...
            <h1 className="text-xl font-bold text-[#d49a3a]" style={{ fontFamily: 'Georgia, serif' }}>Casa Toro</h1>
          </div>
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                    active ? 'bg-[#d49a3a] text-[#2e1e0b] font-semibold' : 'text-stone-400 hover:text-stone-100 hover:bg-white/5'
                  }`}>
